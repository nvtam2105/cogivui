package com.cogivui.controller.social;
...
import com.cogivui.service.social.FacebookUtil;
import com.cogivui.service.social.OAuthServiceProvider;
...
public class FacebookController<FacebookApi> {
    ...
    @ResponseBody
    public String postOnWall(@RequestParam("code") String code, @RequestParam("state") String state, HttpServletRequest request,
	...
	EventOperations eventOperations = template.eventOperations();
	
	PagedList<Event> search = eventOperations.search("Ho Chi Minh");
...
//	} catch (Exception ex) {
//	    LOGGER.error("Exception Occurred while posting a link on facebook for user Id : {}, exception is : {}", userId, ex);
...
//	}
    ...
    public void error(@RequestParam("error_reason") String errorReason, @RequestParam("error") String error,
	    ...
	    throws Exception {
	try {
	    LOGGER.error("Error occurred while validating user, reason is : {}", errorReason);
