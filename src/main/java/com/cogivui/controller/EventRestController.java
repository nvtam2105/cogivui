...
public class EventRestController {
	...
	Status udpateEvent(@PathVariable("id") long id, @RequestBody EventDto eventDto) {
		try {
			...
			event.setId(id);
			eventService.saveOrUpdateOrCloneEvent(event);
	...
	@RequestMapping(value = "/getEventsUpComming/{latitude}/{longitude}/{categoryIds}/{cityId}", method = RequestMethod.GET)
	public @ResponseBody
	List<Event> getEventsUpComing(@PathVariable("latitude") double latitude, @PathVariable("longitude") double longitude,
	        @PathVariable("categoryIds") String categoryIds, @PathVariable("cityId") long cityId) {
		List<Event> events = null;
		try {
			EventCriteria eventCriteria = new EventCriteria();
			eventCriteria.setCategoryIds(ControllerUtils.extractCategoryIds(categoryIds));
			eventCriteria.setCityId(cityId);
			events = eventService.getEventsUpComing(eventCriteria);
			setDistanceForEvents(latitude, longitude, events);
		} catch (Exception e) {
	...
	@RequestMapping(value = "/getEventsUpComming/{latitude}/{longitude}/{categoryIds}/{cityId}/{offset}/{limit}", method = RequestMethod.GET)
	public @ResponseBody
	List<Event> getEventsUpComing(@PathVariable("latitude") double latitude, @PathVariable("longitude") double longitude,
	        @PathVariable("categoryIds") String categoryIds, @PathVariable("cityId") long cityId, @PathVariable("offset") int offset,
	...
	@RequestMapping(value = "/getEventsToday/{latitude}/{longitude}/{categoryIds}/{cityId}", method = RequestMethod.GET)
	public @ResponseBody
	List<Event> getEventsToday(@PathVariable("latitude") double latitude, @PathVariable("longitude") double longitude,
	        @PathVariable("categoryIds") String categoryIds, @PathVariable("cityId") long cityId) {
		List<Event> events = null;
	...
	@RequestMapping(value = "/getEventsToday/{latitude}/{longitude}/{categoryIds}/{cityId}/{offset}/{limit}", method = RequestMethod.GET)
	public @ResponseBody
	List<Event> getEventsToday(@PathVariable("latitude") double latitude, @PathVariable("longitude") double longitude,
	        @PathVariable("categoryIds") String categoryIds, @PathVariable("cityId") long cityId, @PathVariable("offset") int offset,
	        @PathVariable("limit") int limit) {
	...
	@RequestMapping(value = "/getEventsTomorrow/{latitude}/{longitude}/{categoryIds}/{cityId}", method = RequestMethod.GET)
	public @ResponseBody
	List<Event> getEventsTomorrow(@PathVariable("latitude") double latitude, @PathVariable("longitude") double longitude,
	        @PathVariable("categoryIds") String categoryIds, @PathVariable("cityId") long cityId) {
		List<Event> events = null;
	...
	@RequestMapping(value = "/getEventsTomorrow/{latitude}/{longitude}/{categoryIds}/{cityId}/{offset}/{limit}", method = RequestMethod.GET)
	public @ResponseBody
	List<Event> getEventsTomorrow(@PathVariable("latitude") double latitude, @PathVariable("longitude") double longitude,
	        @PathVariable("categoryIds") String categoryIds, @PathVariable("cityId") long cityId, @PathVariable("offset") int offset,
	        @PathVariable("limit") int limit) {
			...
			try {
				...
				for (int i = 1; i < csvRecords.size(); i++) {
					...
					if (StringUtils.isNoneBlank(timeStr)) {
						int hourEndDate = time.getHourStartDate() + 3;
						if (hourEndDate > 12) {
							hourEndDate = hourEndDate - 12;
						}
						if (timeStr.contains("Today") || timeStr.contains(" Tomorrow")) {
							...
							time.setHourEndDate(hourEndDate);
							time.setMinEndDate(time.getMinStartDate());
							time.setEndPeriod(time.getStartPeriod());
							event.setTime(time);
					...
					// String des = StringUtils.isBlank(record.get("desc_full"))
					// ? ControllerUtils.br2nl(record.get("desc_full").trim())
					// :
					// ControllerUtils.br2nl(StringUtils.trim(record.get("desc_short")));

					String descFull = record.get("desc_full");
					event.setExcept(StringUtils.substring(ControllerUtils.html2TextKeepBreakline(descFull), 0, 500));
					
					String des = StringUtils.isNotBlank(descFull) ? descFull.trim() : StringUtils.trim(record.get("desc_short"));
